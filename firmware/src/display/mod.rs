...
fn run(spi: SPI0, clk: PIN_22, mosi: PIN_23, cs: PIN_12, dc: PIN_11, bl: PIN_13) -> ! {
...
async fn metrics_updater() {
    ...
    loop {
        let Metrics { keys_pressed } = sub.next_message_pure().await;
...
pub fn init(
    ...
    spawner.must_spawn(metrics_updater());
