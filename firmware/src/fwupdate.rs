...
pub async fn updater_task(watchdog: WATCHDOG, flash: FLASH) {
    ...
    if updater.get_state_blocking(&mut flash, &mut aligned_buf.0[..1]).unwrap() == embassy_boot_rp::State::Swap {
        updater.mark_booted_blocking(&mut flash, &mut aligned_buf.0[..1]).unwrap();
    }
    ...
    loop {
        ...
        if let Either::First(msg) = select::select(r, t).await {
            match msg {
                FWCmd::Commit => {
                    updater.mark_updated_blocking(&mut flash, &mut aligned_buf.0[..1]).unwrap();
                ...
                FWCmd::WriteChunk { offset, buf } => {
                    ...
                    if let Some(w) = &writer {
                        w.write_blocking(&mut flash, offset, &aligned_buf.0).unwrap();
                    }
                },
            }
        }
